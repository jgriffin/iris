...
/// A labeled section container — the design-language primitive the MODEL and
/// DATASET sidebar sections are built from. Content-agnostic: it owns only the
/// header look (the all-caps `.caption.weight(.semibold)` `.secondary` label),
/// an optional trailing accessory, and optional disclosure collapsibility. The
/// body is supplied by the caller.
///
/// The selectable *mode* sections (Playback / Image / Capture) are NOT built
/// from this — they own their accent-bar + header-band + body look directly in
/// `ModeSection`.
///
/// **Collapsibility.** When `isExpanded` is `nil` there is no chevron and the
/// content always renders. When a binding is supplied the header gains a
/// leading-rotating disclosure chevron and the content shows only while expanded.
struct SidebarSection<Accessory: View, Content: View>: View {
    ...
    /// to preserve its tighter strip layout.
    var spacing: CGFloat
...
/// The shared section-header style: the all-caps `MODEL` / `DATASET` text
/// (`.caption.weight(.semibold)`, `.secondary`), plus an optional trailing
/// accessory and an optional disclosure chevron.
