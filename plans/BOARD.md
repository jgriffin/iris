...
├─ 🗓 P5 — freeze-from-live: "Inspect frame" from playback/capture → Image page  ← here
└─ 🗓 P6 — dataset tie-in: image-shaped `AssetFingerprint` (no `durationSeconds`) + PTS/seek-free `FlaggingSource` + flag→PNG export

👉 next — **M8·P5 — freeze-from-live.** An "Inspect frame" affordance on the playback overlay (and the capture overlay on iOS) that **freezes the visible `Frame`** and opens it in the Image page for model-play. First cross-feature handoff (playback/capture → image); reuses P3's `ImageDetectionCoordinator.setImage`. Acceptance: from a playing or paused video, freeze the visible frame and run other detectors on that still. → [features/M8.md](./features/M8.md) · [LOG.md](./LOG.md)
...
- 🗓 Unified-sidebar nav pass (iOS + macOS) — the user's coming UI pass; the M8·P4 iOS tab + macOS `Videos|Images` toggle are interim seeds it will subsume.
      Direction (from the user's iPhone/iPad screenshots): one cross-platform custom sidebar with **page-rows** (Playback / Image / Capture), a **global MODEL section** at top (model picker + min-confidence) implying a **single shared active-detector** across pages (today each page owns its own `selectedDetectorID` + coordinator), `Open …` + RECENT per page, Dataset/Export pinned bottom; on iPhone the sidebar collapses + the inspector becomes a bottom sheet. The model selector + filter settings move into the sidebar here. (user, 2026-05-29)
- 🗓 macOS stacked `.fileImporter` latent bug — the root view still stacks the **movie + model** importers; SwiftUI honors only one `isPresented` importer per view.
      M8·P4 fixed the *image* importer by moving it to its own view (the images sidebar) after it silently never presented as the 3rd-stacked. Open Video works, but the custom-model importer is the suspect 2nd-stacked — verify it presents on macOS; if not, move it to its own node (or route all via one enum-driven importer). The general rule: **one presentation modifier per view**. (M8·P4, 2026-05-29)
- 🗓 Shared MRU generic — `RecentImages` and `RecentVideos` are near-identical bookmark-backed UserDefaults MRUs; factor a common base. Deliberate siblings for now (M8·P4). Also: `RecentImages` is untested (`Apps/Shared/` test-reachability deferral, as with `RecentVideos`), and macOS custom-model-in-image-mode re-selects the *playback* detector id (minor). (M8·P4, 2026-05-29)
