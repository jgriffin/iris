...
└─ 🔀 M9 — Unified shell — one shared model + a left pane that drives Playback/Image/Capture → [features/unified-sidebar/README.md](./features/unified-sidebar/README.md)  ·  branch `m9-unified-shell` → `main`  ← active
   ...
   ├─ 🔀 P3 — left-pane shell — **shipped (6 steps).** One cross-platform `IrisShell` (`NavigationSplitView` + custom sidebar: MODEL section w/ detector picker + the relocated Min-confidence slider, page-rows that expand inline to Open…/RECENT, reserved disabled DATASET stub) replaces the iOS `TabView` + the macOS Videos/Images toggle; shared `PlaybackDetailView`/`CaptureDetailView`; one shell holds all coordinators (fixes A4 tab-switch reload + A7 scroll-reset); freeze-from-live is now a **direct** call (`InspectorHandoff` deleted); iPad/macOS docked inspector vs iPhone drawer + bottom-sheet.  ·  6 commits on `m9-unified-shell`, both schemes build green — **build-only, hands-on smoke owed**; ⚠ macOS manual dataset export removed (now a stub) — see 🚩 below
...
👉 next — **M9·P4 — Capture joins the shared model** (+ first: settle the 🚩 macOS-export regression + a hands-on smoke of the P3 shell). P1–P3 shipped on `m9-unified-shell` (both schemes build green, unmerged). P4: Capture is still hardcoded to Vision rectangles with no picker (the P3 shell left `CaptureModel.updateDetector(for:)` as a no-op hook) — give it the shared detector + a live detector-swap in its frame loop + the shared min-confidence. Fixes A3. → [features/unified-sidebar/README.md](./features/unified-sidebar/README.md) · [LOG.md](./LOG.md)

🚩 issue → P3 step 2 turned the **working macOS dataset footer** (`Export now` + `Reveal in Finder`) into a **disabled stub**, following the mock's "reserved-but-deferred DATASET slot." But that slot was about *not adding new sidebar export* — it shouldn't have *removed* the macOS export that already worked. Background scenePhase sweeps + the count still run; only the manual buttons are gone. **Decision pending:** restore the macOS manual export controls, or accept the stub until the dataset milestone? (Recommend restore — working things stay working.) (M9·P3·2, 2026-05-30)
